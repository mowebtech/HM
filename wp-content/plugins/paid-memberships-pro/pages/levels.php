	...
	<tr class="<?php if($count++ % 2 == 0) { ?>odd<?php } ?><?php if($current_level == $level) { ?> active<?php } ?>">
		...
		<?php 
				if (!is_user_logged_in()) {?>
					<a href="<?php echo wp_login_url();?>"> Select</a>
				<?php }
				else{
                
                if(empty($current_user->membership_level->ID)) { ?>
                    <?php if($level->name == 'Pro'){?>
                    
                    <a href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>">Select</a>
                    <?php }else{?>    
                    <a href="<?php echo wp_login_url().'&action=profile&level=' . $level->id;?>"> Select</a>
                    <?php }?>
<!--			<a href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>">Select</a>          by user 1     -->
...
<!--			<a href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>">Select</a>       	by user 1		-->
		<?php } elseif($current_level) { ?>      
                    ...
                    <a href="<?php echo wp_login_url().'&action=profile&level=' . $level->id;?>"> Select</a>
                    <?php }?>
<!--			<a href="<?php echo pmpro_url("account")?>">Your&nbsp;Level</a> by user 1-->
		<?php } 
				}//End else
		?>
		</td>
