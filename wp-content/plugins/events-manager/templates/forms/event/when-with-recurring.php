...
<table id="em-form-recurrence" class="event-form-recurrence event-form-when">
	<tr>
		<td width="164px"></td>
		<td><?php _e('This is a recurring event.', 'dbem'); ?> <input type="checkbox" id="em-recurrence-checkbox" name="recurring" value="1" <?php if($EM_Event->is_recurring()) echo 'checked' ?> /></td>
	</tr>
	<tr class="em-date-range">
		<td>
	...
	<tr>
		<td style="vertical-align: top;"><?php _e('To','dbem'); ?></td>
		<td>
			...
			<br/><br/><?php _e('All day','dbem'); ?> <input type="checkbox" class="em-time-allday" name="event_all_day" id="em-time-all-day" value="1" <?php if(!empty($EM_Event->event_all_day)) echo 'checked="checked"'; ?> />
		</td>
	</tr>
	<tr>
		<td></td>
		<td>
			...
			<br/><br/>
			<?php _e ( 'every', 'dbem' )?>
			<input style="width:10% !important" id="recurrence-interval" name='recurrence_interval' size='2' value='<?php echo $EM_Event->interval ; ?>' />
		...
		<br/><br/>
		<span class="alternate-selector" id="monthly-selector" style="display:inline;">
		...
		<br/><br/>
		<span class="em-duration-range">
			<?php _e('Each event lasts','dbem'); ?>
			<input id="end-days" type="text" size="8" maxlength="3" name="recurrence_days" value="<?php echo $EM_Event->recurrence_days; ?>" />
			<?php _e('day(s)','dbem'); ?>
		</span>
		<br/><br/>
		<span class="em-range-description"><em><?php _e( 'For a recurring event, a one day event will be created on each recurring date within this date range.', 'dbem' ); ?></em></span>
	</div>
