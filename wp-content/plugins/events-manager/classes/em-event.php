...
/*end by user2*/
	...
	//start by user2
                'event_types' => array( 'name'=>'event_types', 'type'=>'%s', 'null'=>true ),
                'contact_email' => array( 'name'=>'contact_email', 'type'=>'%s', 'null'=>true ),
	// for premium user-------------------------------------
		'company_name' => array( 'name'=>'company_name', 'type'=>'%s', 'null'=>true ),
		'contact_telephone' => array( 'name'=>'contact_telephone', 'type'=>'%s', 'null'=>true ),
		'website_link' => array( 'name'=>'website_link', 'type'=>'%s', 'null'=>true ),
		'twitter_link' => array( 'name'=>'twitter_link', 'type'=>'%s', 'null'=>true ),
		'facebook_link' => array( 'name'=>'facebook_link', 'type'=>'%s', 'null'=>true ),
		'webtv_url' => array( 'name'=>'webtv_url', 'type'=>'%s', 'null'=>true )		
	// -----------------------------------------------------
	//end by user2
	);
				...
				if(!$is_post){ 
					if( is_numeric($search_by) && is_multisite() ){ 
						...
						$event_post = get_post($id);
                                              //  echo '<pre>'; print_r($event_post); exit;
					}
				}else{
					$event_post = $id;
				}
			...
			if( $event_post->post_status != 'auto-draft' ){
				...
				if( !empty($this->event_rsvp_date ) ){
				    $this->rsvp_end = strtotime($this->event_rsvp_date." ".$this->event_rsvp_time); 
                                ...
                                global $wpdb;
                               	$vent_array = $wpdb->get_row('SELECT * FROM '.EM_EVENTS_TABLE. ' WHERE post_id='.$event_post->ID);
                                /*start by user2*/
                                $this->event_types = $vent_array->event_types;
                                $this->contact_email = $vent_array->contact_email;
				// for premium user ------------------------------
				$this->company_name = $vent_array->company_name;
				$this->contact_telephone = $vent_array->contact_telephone;
				$this->website_link = $vent_array->website_link;
				$this->twitter_link = $vent_array->twitter_link;
				$this->facebook_link = $vent_array->facebook_link;
				$this->webtv_url = $vent_array->webtv_url;
				// -----------------------------------------------
                                /*end by user2*/
	...
	function save_meta(){
		...
		if( ( get_option('dbem_events_anonymous_submissions') && empty($this->event_id)) || $this->can_manage('edit_events', 'edit_others_events') ){
			...
			if( empty($this->event_id) || !$event_truly_exists ){ 
                                /*start by user2*/
                                // to add meta box value to database
                                $event_array['event_types'] = $_POST['event_types'];
                                $event_array['contact_email'] = $_POST['contact_email'];
				//for premium user ----------------------------------
				$event_array['company_name'] = $_POST['company_name'];
				$event_array['contact_telephone'] = $_POST['contact_telephone'];
				$event_array['website_link'] = $_POST['website_link'];
				$event_array['twitter_link'] = $_POST['twitter_link'];
				$event_array['facebook_link'] = $_POST['facebook_link'];
				$event_array['webtv_url'] = $_POST['webtv_url'];
				// --------------------------------------------------
                                /*end by user2*/
