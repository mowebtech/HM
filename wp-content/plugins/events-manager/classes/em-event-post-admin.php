...
class EM_Event_Post_Admin{
	...
	function save_post($post_id){    
		...
		if(!defined('UNTRASHING_'.$post_id) && $is_post_type && $saving_status ){
			if( !empty($_REQUEST['_emnonce']) && wp_verify_nonce($_REQUEST['_emnonce'], 'edit_event') ){ 
				...
				$EM_Event = em_get_event($post_id, 'post_id');
                                ...
                                /*start by user2*/
                                //add_action( 'em-event-demo', 'meta_box_demo_save' ); 
                                /*end by user2*/				
        ...
        /*start by user2*/
        /*
        function meta_box_demo_save( $post_id ) 
        { echo 'her'; exit;
            // Bail if we're doing an auto save  
            if( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return; 
     
            // if our nonce isn't there, or we can't verify it, bail 
            if( !isset( $_POST['_demo'] ) || !wp_verify_nonce( $_POST['_demo'], '_demo' ) ) return; 
	...
	function meta_boxes(){ 
		...
		if( !empty($EM_Event->event_owner_anonymous) ){
			add_meta_box('em-event-anonymous', __('Anonymous Submitter Info','dbem'), array('EM_Event_Post_Admin','meta_box_anonymous'),EM_POST_TYPE_EVENT, 'side','high');
                ...
                /*start by user2*/
               // add_meta_box('em-event-demo', __('Demo','dbem'), array('EM_Event_Post_Admin','meta_box_demo'),EM_POST_TYPE_EVENT, 'side','high');
		/*end by user2*/
	...
	function meta_box_date(){
        ...
        /*start by user2*/
        /*
        function meta_box_demo(){
            //create demo
            ?>
                <input type="text" name="_demo" value="" />
            <?php
        }         
        */
        /*end by user2*/
...
/**
 * To add event_types meta box 
 */
function my_em_types_meta_boxes(){ 
	add_meta_box('em-event-types', 'Types', 'my_em_types_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_types_meta_boxes');

function my_em_types_metabox(){ 
	global $EM_Event;
       // echo $EM_Event->event_types;
       // print_r($EM_Event); 
        ?>           
...
/**
 * To add contact email meta box 
 */
function my_em_contact_email_meta_boxes(){ 
	add_meta_box('em-contact_email', 'Contact Email', 'my_em_contact_email_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_contact_email_meta_boxes');

function my_em_contact_email_metabox(){ 
	global $EM_Event;
       // echo $EM_Event->event_types;
       // print_r($EM_Event); 
        ?>           
            <input type="text" name="contact_email" value="<?php echo $EM_Event->contact_email; ?>" />
        <?php
}


//Starts Fields for premium users only ----------------------------------------------------------
/**
 * To add company name field
 */
function my_em_company_name_meta_boxes(){ 
	add_meta_box('em-company_name', 'Company', 'my_em_company_name_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_company_name_meta_boxes');

function my_em_company_name_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="company_name" value="<?php echo $EM_Event->company_name; ?>" />
        <?php
}


/**
 * To add Contact telophone number
 */
function my_em_contact_telephone_meta_boxes(){ 
	add_meta_box('em-contact_telephone', 'Contact Telephone Number', 'my_em_contact_telephone_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_contact_telephone_meta_boxes');

function my_em_contact_telephone_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="contact_telephone" value="<?php echo $EM_Event->contact_telephone; ?>" />
        <?php
}

/**
 * To add Website Link meta box
 */
function my_em_website_link_meta_boxes(){ 
	add_meta_box('em-website_link', 'Website Link', 'my_em_website_link_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_website_link_meta_boxes');

function my_em_website_link_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="website_link" value="<?php echo $EM_Event->website_link; ?>" />
        <?php
}

/**
 * To add Twitter Link meta box
 */
function my_em_twitter_link_meta_boxes(){ 
	add_meta_box('em-twitter_link', 'Twitter Link', 'my_em_twitter_link_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_twitter_link_meta_boxes');

function my_em_twitter_link_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="twitter_link" value="<?php echo $EM_Event->twitter_link; ?>" />
        <?php
}

/**
 * To add FB Link meta box
 */
function my_em_facebook_link_meta_boxes(){ 
	add_meta_box('em-facebook_link', 'Facebook Link', 'my_em_facebook_link_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_facebook_link_meta_boxes');

function my_em_facebook_link_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="facebook_link" value="<?php echo $EM_Event->facebook_link; ?>" />
        <?php
}

/**
 * To add Web tv url  meta box
 */
function my_em_webtv_url_meta_boxes(){ 
	add_meta_box('em-webtv_url', 'Web TV Url', 'my_em_webtv_url_metabox',EM_POST_TYPE_EVENT, 'side','low');
}
add_action('add_meta_boxes', 'my_em_webtv_url_meta_boxes');

function my_em_webtv_url_metabox(){ 
	global $EM_Event;
        ?>           
            <input type="text" name="webtv_url" value="<?php echo $EM_Event->webtv_url; ?>" />
        <?php
}


//End Fields for premium users only -------------------------------------------------------------
