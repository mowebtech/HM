...
<div class="login" id="theme-my-login<?php $template->the_instance(); ?>">
    ...
    <form name="registerform" id="registerform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'register' ); ?>" method="post">
        ...
        <p>
            <label for="user_email<?php $template->the_instance(); ?>"><?php _e( 'Reconfirm E-mail Address', 'theme-my-login' ) ?></label>
            <input type="text" name="user_email_reconfirm" id="user_email<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'user_email_reconfirm' ); ?>" size="20" />
        </p>
	<?php do_action_ref_array( 'tml_register_form', array( &$template ) ); //TML hook?>        
        <p>
            
            <input type="checkbox" name="terms" id="terms<?php $template->the_instance(); ?>" class="input" value="1" <?php checked($template->the_posted_value( 'terms' )); ?>/>
