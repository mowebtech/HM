				...
				case 'register' :
					...
					if ( $http_post ) {
						...
						//$errors = Theme_My_Login::register_new_user( $user_login, $user_email );
                                                $errors = Theme_My_Login::register_new_user( $_POST );
						...
						exit();
                                        ...
                                        if (!is_wp_error( $errors ) ) {
                                               $user = wp_get_current_user();
                                                ...
                                                $redirect_to = Theme_My_Login::get_current_url().'&action=image&level='.$_POST['level'].'&user_id='.$user->ID;
...
<?php
	...
	/*function register_new_user( $user_login, $user_email ) {
        ...
        function register_new_user( $post ) {
		$errors = new WP_Error();
             /*  echo "<pre>";
               print_r($_POST);
               exit;*/
		
                $sanitized_user_login = sanitize_user( $post['user_login'] );
		...
		if ( $sanitized_user_login == '' ) {
			...
			$errors->add( 'username_exists', __( '<strong>ERROR</strong>: This username is already registered, please choose another one.', 'theme-my-login' ) );
                ...
                //post email
		$user_email = apply_filters( 'user_registration_email',  $post['user_email']  );
                $user_email_reconfirm = apply_filters( 'user_registration_email',  $post['user_email_reconfirm']  );
		...
		if ( '' == $user_email_reconfirm ) {
			$errors->add( 'empty_email', __( '<strong>ERROR</strong>: Please type your re confirm e-mail address.', 'theme-my-login' ) );
		} elseif ( !is_email( $user_email_reconfirm ) ) {
			$errors->add( 'invalid_email', __( '<strong>ERROR</strong>: The re confirm email address isn&#8217;t correct.', 'theme-my-login' ) );
			$user_email = '';
		} elseif ($user_email != $user_email_reconfirm ) {
			$errors->add( 'invalid_email', __( '<strong>ERROR</strong>: This email and re confirm email is not same.', 'theme-my-login' ) );
		}
                
                
                //check terms and condition click or not
                if ( '' == $terms ) {
                        
                        $errors->add( 'empty_terms', __( '<strong>ERROR</strong>: Please check Terms and Condition.', 'theme-my-login' ) );
		...
		$user_pass = apply_filters( 'tml_user_registration_pass', $post['pass1']);
	 ...
	 * @return int|WP_Error Either user's ID or error on failure.
         * by user 1
	 */
        function profile_update_step1($post)
        {
            ...
            $errors = new WP_Error();
             ...
             $mebership_id = $post['level'];
             $dateofbirth = $post['dateofbirth'];
             $about_you = $post['about_you'];
             ...
             $dire_to_venue = $post['dire_to_venue'];
             $email = $post['email'];
             $url = $post['url'];
             $aim = $post['aim'];
             $yim = $post['yim'];
             $jabber = $post['jabber'];
             $telephone_num = $post['telephone_num'];
             $opening_hour = $post['opening_hour'];
             
            // Check the location
            if ( $location == '' ) {
                    $errors->add( 'empty_location', __( '<strong>ERROR</strong>: Please enter a location.', 'theme-my-login' ) );
                    return $errors;
            } 
             ...
             if($mebership_id!='')
             {    
                if($wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->pmpro_memberships_users WHERE user_id = %s", $user_id ) ))
                {

                    //$wpdb->update( $wpdb->pmpro_memberships_users, array( 'user_id' => $user_id,'membership_id'=>$mebership_id ));
                    $wpdb->update( $wpdb->pmpro_memberships_users, array( 'membership_id'=>$mebership_id ), array( 'user_id' => $user_id) );
                }else{
