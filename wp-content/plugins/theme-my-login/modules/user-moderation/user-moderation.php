...
class Theme_My_Login_User_Moderation extends Theme_My_Login_Module {
	...
	function user_activation() {
		...
		if ( !is_wp_error( $errors ) ) {
                        //by user 1
                        $user_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->users WHERE user_login = %s", $_GET['login'] ) );
                        $creds = array();
                        $creds['user_login'] = $user_data->user_login;
                        $creds['user_password'] = $_GET['key'];
                        $creds['remember'] = true;
                        ...
                        $redirect_to = apply_filters("pmpro_register_redirect", pmpro_url("levels"));//By user 1
			//$redirect_to = Theme_My_Login::get_current_url( 'activation=complete' );//By user 1
